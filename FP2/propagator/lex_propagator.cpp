...
{
	return out << format("adv(%1%, %2%)") % prop->getName() % idx;
}
	...
	: Propagator(d), x(_x), y(_y)
{
	...
	// std::cout << boost::format("alpha = %1% beta = %2%") % alpha % beta << std::endl;
	if (alpha >= beta)
	{
		state = CSTATE_INFEAS;
		return;
...
{
	return out << format("LexLessEqualProp(%1%, %2%, alpha=%3%, beta=%4%, n=%5%)")
		% name
