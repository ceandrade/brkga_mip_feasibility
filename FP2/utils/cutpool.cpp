...
void Cut::digest()
{
	sig = boost::hash_range(row.idx(), row.idx() + row.size());
...
CutPool::CutPool()
		...
		maxMemory(MAX_MEMORY_DEF), maxSize(MAX_SIZE_DEF), logConfig(false),
...
bool CutPool::select(const std::vector<double>& x, CutList& cuts)
{
	...
	{
		dominiqs::numarray<double> ar(x.size());
...
void CutPool::readConfig(XMLConfig& config, const std::string& root)
{
	...
	if (logConfig)
	{
		GlobalAutoSection config("config", "name", name);
		gLog().logItem(root + ".maxSeparated", maxSeparated);
		gLog().logItem(root + ".minEfficacy", minEfficacy);
		gLog().logItem(root + ".minOrtho", minOrtho);
		gLog().logItem(root + ".maxMemory", maxMemory);
		gLog().logItem(root + ".maxSize", maxSize);
	}
}
